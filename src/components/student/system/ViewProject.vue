...
export default {
  ...
  created() {
    ...
    firebase.database().ref('result_register/' + this.result.year).on('child_added', snapshot => {
    ...
    firebase.database().ref('teacher/' + this.register.teacher).once('value').then(snapshot => {
      this.teacher = {
        name: snapshot.val().prefix + ' ' + snapshot.val().firstname + ' ' + snapshot.val().lastname
      }
    })
  }
};
